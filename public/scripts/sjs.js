function sendButton() {
  ...
  // };
      ...
      console.log('prompt>>'+userMessage)
        ...
        fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                "messages": [
                    {"role": "system", "content": "As a master Mixologist make a tasty blend" }, //As an expert provide study material for 40hours of online course with tests
        ...
        .then(response => {
          ...
          // waitImg.src="";
          let array2 = response.choices[0].message.content;
          const searchValue = 3;
          ...
          var array = response.choices[0].message.content.split('\n').filter(line => line.trim() !== '');
          ...
          // }
            ...
            if (array.length > 0) {
              for (j in array) {
                ...
                // if (charlen > 121 ) {
                inf.setAttribute("style", "width:100%; border:1px solid");
            ...
            btn.setAttribute("class", "accordion");
            btn.setAttribute("id", bid);
            btn.setAttribute("onclick","createContent()");
            var t = document.createTextNode('save content');
            btn.appendChild(t);
            //para.appendChild(btn);
            document.getElementById("tcon").appendChild(btn);
        })
    ...
    function myFunction() {
      ...
      if(dFlag){
        // dFlag = true;
        userMessage = divValue; //+' explain in a simple words keeping in context the title of the course';
        responsiveVoice.speak(divValue, "UK English Male");
        const apiKey = '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'; 
        // getTopicDetails(''+)
          ...
          fetch('https://api.openai.com/v1/chat/completions', {
              ...
              body: JSON.stringify({
                  "model": "gpt-3.5-turbo", // Corrected model name
                  "messages": [
                      {"role": "system", "content": "As a Master Instructor explain in a simple words keeping in context the title of the course" },
                      {"role": "user", "content": userMessage}
                  ],
                  max_tokens: 4000 // Adjust as needed
              })
          ...
          .then(response => {
            ...
            // const botResponse = JSON.stringify(response, null, 2);
            var arraymf = response.choices[0].message.content.split('\n').filter(line => line.trim() !== '');
